...
/**
 * A logging class that supports ETL's String and is lightweight enough to be used in embedded
 * development.
 *
 ...
 * @endcode
 * In the above example, if `getTemperature()` will cause a side effect (e.g. a `std::cout` print),
